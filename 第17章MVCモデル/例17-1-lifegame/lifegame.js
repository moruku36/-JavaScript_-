...
var view     = Object.create(null); // V:状態をグラフィックで表示するオブジェクト
var controls = Object.create(null); // C:コントローラーオブジェクト
//
//  * stateとviewは次の独自イベントで疎結合
//         view  -> state : clickviewイベント（viewがクリックされたとき発生）
//         state -> view  : changeCellイベント（セルの値が変更されたとき発生）
//                          changeGenerationイベント（世代数が変更されたとき発生）
//  * controlsはstate,viewの操作（パラメータの変更、メソッドの実行）を行う
//         controls -> state, view
//
/*===========================================================================*
 * 初期化処理
 *    パターンを格納したJSONファイルを読み込み、ライフゲームを生成してbody要素内に配置
 *===========================================================================*/
 ...
 * ユーティリティ関数
 *===========================================================================*/
// * ファイルfilenameを読み込み、内容をJSONオブジェクトとしてcallback関数で返す
...
// 以下state, view, controlsの3つのオブジェクトを定義する
/*===========================================================================*
 * [ stateオブジェクト ]：生物の棲む格子と生物の状態を格納するオブジェクト
 *===========================================================================*/
// * stateオブジェクトにプロパティを定義し、イベントリスナを登録する
...
// * セルの変更時に呼ぶメソッドで、changecellイベントを発生させる
...
// * 世代数の変更時に呼ぶメソッドで、changegenerationイベントを発生させる
...
// * セルの回りの生物の数の合計を求める（格子は周期的境界条件を満たすとする）
...
state.update = function() {
	...
	// 全セルのスキャン後に、セルの変更を行う（排他的論理和により0->1,1->0とする）
...
/*===========================================================================*
 * [ viewオブジェクト ]：画面表示用オブジェクト
 *===========================================================================*/
// * 横nx、縦nyの格子、横width、縦heightのピクセル数の格子あらわすオブジェクトviewを生成
//   次の２つのcanvas要素を生成し、絶対配置で重ねて表示する
//      view.layer[1]は生物を表示するcanvas要素
//      view.layer[0]は格子線表示用のcanvas要素
view.create = function(nx, ny, width, height) {
	// レイヤーをあらわすcanvas要素を生成
	view.layer = [];
	// 生物表示用
	...
	view.layer[1].addEventListener("click", function(e) {
		...
		var iy = Math.floor(e.offsetY/view.cellHeight);	// セルのy方向の番号
...
// * stateオブジェクトからコールバックでセルに生物を描画する
...
// * stateオブジェクトからコールバックで世代数を表示する
view.showGeneration = function(generation) {
	view.generation.innerHTML = generation;
};
/*===========================================================================*
 * [ controls ]：コントロールオブジェクト
 *    メソッドのみをもつ。各メソッドは要素オブジェクトを生成+初期化して返り値として返す。
 *    それと同時に、その要素オブジェクトにイベントリスナを登録する。
 *    メソッド毎の要素は、createLifeGame関数によって自動的にdiv.toolbarの子要素として
 *    挿入される。
 *===========================================================================*/
// * 連続再生ボタン
