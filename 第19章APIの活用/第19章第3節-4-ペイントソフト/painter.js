/*--------------------------------------------------------------------------------*
 * 画面を構成する要素を生成し、各要素のイベントリスナーを登録する
 *--------------------------------------------------------------------------------*/
 ...
 *   各コントローラは、controlsオブジェクトのメソッドとして登録されています。
...
var colorInput;	// Firefox用のchangeイベント対策用にinput[type="color"]オブジェクト格納
// * ペイントツールの選択
...
// * 色の選択（線と塗りの両方設定 → 必要に応じて別々のコントロールにして下さい）
...
controls.filter = function(ctx) {
	...
	select.addEventListener("change", function(e) {
		...
		// フィルターツールを実行し、使用したワーカーを受けとる
		var worker = filterTools[filterTool](inputImage);
		// ワーカーからの処理結果をメッセージから受けとり描画する
